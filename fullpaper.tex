...
\begin{abstract}
%1. State the problem
Distributed Hash Tables typically assign nodes a single identifier derived from the hash of their IP address and port, which results in a random and unoptimized overlay.   
%2. Say why it’s an interesting problem
Suppose nodes are assigned additional identifiers based on attributes.  It would then be possible to construct an overlay optimized over some of these attributes.
%3. Say what your solution achieves
We developed VHash, a DHT for a multi-dimensional overlay based on a fast and efficient Delaunay Triangulation approximation.  Each dimension in the overlay corresponds to an attribute we wish to optimize over. 
%4. Say what follows from your solution
Using VHash, we were able to embed our overlay with edges that minimized latency and outperform Chord, while still providing a robust, scalable, and efficient distributed lookup service.
